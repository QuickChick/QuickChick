...
(* Sample (genListSized 4 (choose(0,3))).

    [] *)

(** **** Exercise: 3 stars, standard, optional (genColorOption)
...
(* SOLUTION: *)
Definition gen_opt_color : G (option color) :=
  freq [(3, ret None);
        (15, ret (Some Red));
        (5, ret (Some Green));
        (5, ret (Some Blue));
...
(* SOLUTION: *)
Fixpoint shrinkTernTree {A} (s : A -> list A) (t : TernaryTree A) 
  : list (TernaryTree A) :=
  match t with
    | TLeaf => []
    | TNode x l m r => 
      l :: m :: r ::
        ...
        map (fun l' => TNode x l' m r) (shrinkTernTree s l) ++
        map (fun m' => TNode x l m' r) (shrinkTernTree s m) ++
        map (fun r' => TNode x l m r') (shrinkTernTree s r)
  end.
(** [] *)
...
(* QuickChick
  (forAllShrink
     (genTernTreeSized 3 (choose(0,1)))
     (shrinkTernTree shrink)
     (fun t => forAll genPath (fun p =>
     tern_mirror_path_flip t p))). 
...
(** **** Exercise: 2 stars, standard, optional (tern_tree_typeclasses)

    Add typeclass instances for [GenSized] and [Shrink] so that you
    can [QuickChick tern_mirror_path_flip] directly. *)

(* SOLUTION: *)
#[export] Instance gen_sized_tern_tree {A} `{Gen A} : GenSized (TernaryTree A) :=
  { arbitrarySized s := genTernTreeSized s arbitrary }.

#[export] Instance shrink_tern_tree {A} `{Shrink A} : Shrink (TernaryTree A) :=
  { shrink t := shrinkTernTree shrink t }.
...
(* SOLUTION: *)
Fixpoint genTreeSized'' {A : Type} (sz : nat) (g : G A) : G (Tree A) :=
  match sz with
  | 0 => ret Leaf
  | S sz' =>
      freq [ (1, ret Leaf) ;
             (sz*sz, liftM3 Node g (genTreeSized'' sz' g) (genTreeSized'' sz' g)) ]
  end.
(** [] *)
...
(* SOLUTION: *)
Definition inspect_spec_sorted' (x : nat) :=
  forAllShrink (genSortedList 0 5 5) shrink (fun l => collect
               l (insert_spec x l)).

(* QuickChick inspect_spec_sorted'.

     ===> Way more 5's than not! 
*)

(* Descending generator *)
Fixpoint genSortedListDesc (low high : nat) (size : nat)
           : G (list nat) :=
  match size with
  | O => ret []
  | S size' =>
    if high <? low then
      ...
      freq [ (1, ret []) ;
             (size, x <- choose (low, high);;
                    xs <- genSortedListDesc low x size';;
                    ret (x :: xs)) ]
  end.

(* Sample (genSortedListDesc 0 5 5). *)

Definition genSorted' low high size := 
  oneOf [ genSortedList low high size ;
          liftGen (@List.rev nat)
                  (genSortedListDesc low high size) ].

Definition inspect_spec_sorted'' (x : nat) :=
  forAllShrink (genSorted' 0 5 5) shrink
               (fun l => collect l (insert_spec x l)).

(* QuickChick inspect_spec_sorted''. *)

(* Another solution would be to generate a list and sort it.
