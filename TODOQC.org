...
Cosmetic:
  - Colorizing the output would be REALLY helpful for readability.  We'll
    want to make it switch-selectable so that people using emacs compilation
    mode, for example, don't get messed up by it.  But in fact this can be
    fixed: 
