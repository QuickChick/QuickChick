HIGH PRIORITY:
  ...
  - Make the QC command fail if the testing fails
...
MINOR:
  - Figure out why 'make install' recompiles so many files all the time
  - Add
...
COSMETIC:
  - Colorizing the output (using some ansi terminal package) might be
    helpful for readability.  We'll want to make it switch-selectable so
    that people using emacs compilation mode, for example, don't get messed
    up by it.  But this may be fixable:
