...
(* Instead of writing a generator, shrinker, and printer for trees, we
   could simply derive them using the `QCDerive` command.
...
QCDerive Checker for (balanced n t).
(* ==> DecOptbalanced is defined *)

(* This QCDerive command produces an instance of the DecOpt typeclass for
...
(* Sure enough, not all trees are balanced. But how would we go about generating 
   balanced trees for testing purposes? Another `QCDerive` command to the rescue! *)

QCDerive Generator for (fun t => balanced n t).
(* ==> GenSizedSuchThatbalanced is defined *)

(* This `QCDerive` command produces an instance of the GenSizedSuchThat typeclass,
