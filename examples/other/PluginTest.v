...
QCDerive ArbitrarySizedSuchThat for (fun p => let (m,tr) := p in recursion_test m tr).
...
QCDerive DecOpt for (checker_test n t).

QCDerive ArbitrarySizedSuchThat for (fun p => let (m,tr) := p in checker_test m tr).

QCDerive ArbitrarySizedSuchThat for (fun tr => recursion_test m tr).
