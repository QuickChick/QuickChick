...
(** val randomRBool : (bool * bool) -> randomSeed -> bool * randomSeed **)

let randomRBool = QuickChickLib.randomRBool
...
(** val chooseBool : bool choosableFromInterval **)

let chooseBool =
  { super = ordBool; randomR = randomRBool }
...
let genBoolSized _ =
  GenLow.choose chooseBool (false, true)
...
let gen_memory =
  GenHigh.vectorOf (Pervasives.succ (Pervasives.succ 0)) gen_atom
...
(** val ainstr : state0 -> instruction GenLow.coq_G **)

let ainstr st =
  let { st_imem = _; st_mem = _; st_stack = stk; st_pc = _ } = st in
  let stack_length =
  ...
  GenHigh.freq_ (GenLow.returnGen Nop) (((Pervasives.succ 0),
    (GenLow.returnGen Nop)) :: (((Pervasives.succ (Pervasives.succ
    (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
    (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
    0)))))))))),
    (GenHigh.liftGen (fun x -> Push x) gen_Z)) :: (((Pervasives.succ
    (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
    (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
    (Pervasives.succ 0)))))))))),
    (GenHigh.liftGen (fun x -> BCall x)
      (if (=) sl 0
       ...
       else GenLow.choose chooseZ (0, (Z.sub (Z.of_nat sl) 1))))) :: (((
    if containsRet stk
  ...
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    GenHigh.freq_ (GenLow.returnGen Mty) (((Pervasives.succ (Pervasives.succ
      ...
      (GenHigh.liftGen2 (fun x x0 -> Cons0 (x, x0)) gen_atom
        (gen_stack n' onlyLow))) :: (((Pervasives.succ (Pervasives.succ
      (Pervasives.succ (Pervasives.succ 0)))),
      (GenLow.bindGen gen_atom (fun pc ->
        GenHigh.liftGen (fun x -> RetCons (pc, x))
          (gen_stack n' (is_atom_low pc))))) :: [])))
    n
...
let gen_state =
  ...
  GenLow.bindGen gen_atom (fun pc ->
    GenLow.bindGen gen_memory (fun mem1 ->
      GenLow.bindGen
        (gen_stack (Pervasives.succ (Pervasives.succ (Pervasives.succ
          (Pervasives.succ 0)))) (is_atom_low pc)) (fun stk ->
        GenLow.bindGen
          (ainstr { st_imem = imem0; st_mem = mem1; st_stack = stk; st_pc =
            pc }) (fun i ->
          GenLow.returnGen { st_imem = (i :: (i :: [])); st_mem = mem1;
            st_stack = stk; st_pc = pc }))))
...
type 'a vary =
  'a -> 'a GenLow.coq_G
  (* singleton inductive, whose constructor was Build_Vary *)

(** val vary0 : 'a1 vary -> 'a1 -> 'a1 GenLow.coq_G **)

let vary0 vary1 =
  vary1

(** val vary_atom : atom vary **)

let vary_atom a = match a with
| Atm (_, l) ->
  (match l with
   | L -> GenLow.returnGen a
   | H ->
     GenHigh.liftGen2 (fun x x0 -> Atm (x, x0)) gen_Z (GenLow.returnGen H))

(** val vary_mem : mem0 vary **)

let vary_mem m =
  GenHigh.sequenceGen (map (vary0 vary_atom) m)

(** val vary_stack : stack -> bool -> stack GenLow.coq_G **)

let rec vary_stack s isLow =
  match s with
  | Mty -> GenLow.returnGen Mty
  | Cons0 (a, s') ->
    if isLow
    then GenHigh.liftGen2 (fun x x0 -> Cons0 (x, x0)) (vary0 vary_atom a)
           (vary_stack s' isLow)
    else GenHigh.liftGen2 (fun x x0 -> Cons0 (x, x0)) gen_atom
           (vary_stack s' isLow)
  | RetCons (pc, s') ->
    let Atm (x, l) = pc in
    (match l with
     | L ->
       GenHigh.liftGen (fun x0 -> RetCons ((Atm (x, l)), x0))
         (vary_stack s' true)
     | H ->
       GenHigh.liftGen2 (fun x0 x1 -> RetCons (x0, x1))
         (vary0 vary_atom (Atm (x, l))) (vary_stack s' isLow))

(** val vary_state : state0 vary **)

let vary_state st =
  let { st_imem = imem; st_mem = mem1; st_stack = stk; st_pc = pc } = st in
  GenLow.bindGen (vary0 vary_mem mem1) (fun mem' ->
    GenLow.bindGen (vary0 vary_atom pc) (fun pc' ->
      let isLow = let Atm (_, l) = pc in (match l with
                                          | L -> true
                                          | H -> false)
      in
      if isLow
      then GenLow.bindGen (vary_stack stk isLow) (fun stk' ->
             GenLow.returnGen { st_imem = imem; st_mem = mem'; st_stack =
               stk'; st_pc = pc' })
      else GenLow.bindGen (vary_stack stk isLow) (fun stk' ->
             GenLow.bindGen (arbitrary (genOfGenSized genBoolSized))
               (fun b ->
               if b
               then GenLow.bindGen gen_atom (fun extra_elem ->
                      ret (Obj.magic GenLow.coq_Monad_G) { st_imem = imem;
                        st_mem = mem'; st_stack = (Cons0 (extra_elem, stk'));
                        st_pc = pc' })
               else ret (Obj.magic GenLow.coq_Monad_G) { st_imem = imem;
                      st_mem = mem'; st_stack = stk'; st_pc = pc' }))))

(** val gen_variation_state : state0 variation GenLow.coq_G **)

let gen_variation_state =
  GenLow.bindGen gen_state (fun st ->
    GenLow.bindGen (vary0 vary_state st) (fun st' ->
      GenLow.returnGen (V (st, st'))))
              ...
              | H ->
                (match exec t2 st1 with
                 | Some st1' ->
                   (match exec t2 st2 with
                    | Some st2' ->
                      if (&&) (is_atom_low0 st1'.st_pc)
                           (is_atom_low0 st2'.st_pc)
                      then res.exp_check (indist0 indist_state st1' st2')
                      else if is_atom_low0 st1'.st_pc
...
let prop p gen0 t2 r =
  forAllShrink testChecker (showOpt (show_var show_state_pair)) gen0
    (fun _ -> []) (fun mv ->
    match mv with
    | Some v -> p t2 v r
    | None -> r.exp_reject)
...
(** val prop_SSNI_smart : table -> exp_result -> checker **)

let prop_SSNI_smart t2 r =
  prop sSNI (GenHigh.liftGen (fun x -> Some x) gen_variation_state) t2 r

(** val quickchick : unit -> result0 * char list **)

let quickchick _ =
  let _qc_res =
    fuzzCheck testChecker (testMutantX prop_SSNI_smart exp_result_fuzz 1)
  in
