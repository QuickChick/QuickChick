...
(** val gen_Z0 : int GenLow.coq_G **)

let gen_Z0 =
  GenLow.choose chooseZ (0, mem_length)

(** val gen_label0 : label0 GenLow.coq_G **)

let gen_label0 =
  GenHigh.elems_ L (L :: (H :: []))

(** val gen_atom0 : atom GenLow.coq_G **)

let gen_atom0 =
  GenHigh.liftGen2 (fun x x0 -> Atm (x, x0)) gen_Z0 gen_label0
...
let gen_memory =
  GenHigh.vectorOf (Pervasives.succ (Pervasives.succ (Pervasives.succ
    (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
    (Pervasives.succ (Pervasives.succ (Pervasives.succ 0)))))))))) gen_atom0
...
let ainstr st fuel =
  ...
  GenHigh.freq_ (GenLow.returnGen Nop) (((Pervasives.succ 0),
    ...
    (Pervasives.succ (Pervasives.succ
    0)))))))))))))))))))))))))))))))))))))))),
    (GenHigh.liftGen (fun x -> Push x) gen_Z0)) :: (((if dec0
                                                           (dec_class_dec
                                                             (decidable_le_nat
                                                               (Pervasives.succ
                                                               sl)
...
let rec gen_stack n onlyLow =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    GenHigh.freq_ (GenLow.returnGen Mty) (((Pervasives.succ (Pervasives.succ
      ...
      (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
      0)))))))))),
      (GenHigh.liftGen2 (fun x x0 -> Cons0 (x, x0)) gen_atom0
        (gen_stack n' onlyLow))) :: (((Pervasives.succ (Pervasives.succ 0)),
      (GenLow.bindGen gen_atom0 (fun pc ->
...
(** val gen_variation_medium : state0 variation option GenLow.coq_G **)

let gen_variation_medium =
  GenLow.bindGen gen_state' (fun st1 ->
    GenLow.bindGen (vary0 vary_state st1) (fun st2 ->
      if indist0 indist_state st1 st2
...
(** val prop_SSNI_medium : table -> exp_result -> checker **)

let prop_SSNI_medium t2 r =
  prop sSNI gen_variation_medium t2 r

(** val quickchick : unit -> result0 * char list **)

let quickchick _ =
  let _qc_res =
    fuzzCheck testChecker (testMutantX prop_SSNI_medium exp_result_fuzz 1)
  in
