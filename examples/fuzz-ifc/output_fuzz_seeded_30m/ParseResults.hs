...
data Mode = Naive | Medium | Smart | Seeded
...
fileNameP = do
  string "fuzz_"
  s <- string "seeded_SSNI_" <|> string "SSNI_"
  m0 <- smode <$> (try (string "arbitrary") <|> string "arbmedium" <|> try (string "smart"))
  let m = if head s == 's' then Seeded else m0
  char '_'
  xs <- many1 digit
  let x = read xs :: Int
  return (m, x)
...
parseOutput fn allLines =
  ...
  in MkData m id execsDone unique total 1800
