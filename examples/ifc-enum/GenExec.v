...
Definition ainstr (st : State) : G Instruction :=
  ...
  let possibleInstructions : G Instruction :=
      let bcall := if sl < 1 ? then nil else ret (liftGen BCall (if beq_nat sl 0 then returnGen 0
                                                                 else choose (0, Z.of_nat sl-1))%Z) in
      let bret := if containsRet stk then nil else ret (returnGen BRet) in
      let add := if sl < 2 ? then nil else ret (returnGen Add) in
      let load := if sl < 1 ? then nil else ret (returnGen Load) in
      let store := if sl < 2 ? then nil else ret (returnGen Store) in
      sumG (list_to_LazyList (bcall ++ bret ++ add ++ load ++ store))
  in
  freq_ (returnGen Nop) [
              (1, returnGen Nop);
              (10, liftGen Push gen_Z);
              (40, possibleInstructions)].
