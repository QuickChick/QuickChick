...
QCDerive ArbitrarySizedSuchThat for (fun z => good_Z z).

(* Definition gen_label := elements L [L; H]. *)
QCDerive Arbitrary for Label.
...
QCDerive ArbitrarySizedSuchThat for (fun a => good_atom a).
...
QCDerive ArbitrarySizedSuchThat for (fun m => good_mem m).
...
QCDerive ArbitrarySizedSuchThat for (fun s => good_stack n s).
...
QCDerive ArbitrarySizedSuchThat for (fun n => stack_length s n).
...
QuickChickWeights [ (GoodNop, 1)
                  ...
                  ; (GoodLoad, 10)
                  ; (GoodStore, 100) ].
QCDerive ArbitrarySizedSuchThat for (fun i => good_instr stk i).
...
QCDerive ArbitrarySizedSuchThat for (fun st => good_state st).
...
QCDerive ArbitrarySizedSuchThat for (fun y => variation_atom x y).
...
QCDerive ArbitrarySizedSuchThat for (fun m2 => variation_mem m1 m2).
...
QCDerive ArbitrarySizedSuchThat for (fun s2 => variation_stack s1 s2).
...
QCDerive ArbitrarySizedSuchThat for (fun stk' => variation_high_stack pc stk stk').
...
QCDerive ArbitrarySizedSuchThat for (fun st' => variation_state st st').
