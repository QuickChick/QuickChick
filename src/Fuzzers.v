...
Inductive FuzzType (ST : Type) (A:Type) : Type :=
  MkFuzz : (ST -> nat -> RandomSeed -> A) -> FuzzType ST A.

Definition ST A := list A.
Definition G A := FuzzType (ST A) A.

(** * Primitive generator combinators *)
  
Definition run {A : Type} (g : G A) :=
  match g with
    MkFuzz f => f end.
...
Definition bindGen {ST A B : Type} (g : G A) (k : A -> G B) : G B :=
  MkFuzz (fun st n r =>
            let (r1,r2) := randomSplit r in
            run (k (run g st n r1)) st n r2).
