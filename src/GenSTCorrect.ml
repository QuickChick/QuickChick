...
type btyp = (coq_expr * coq_expr * (coq_expr -> coq_expr))
...
let ret_exp (c : coq_expr) : btyp =
  (set_singleton c,
   returnGenSizeMonotonic (gSome c),
   fun hcur -> rewrite (imset_singl_incl hole hole hole hcur) (rewrite_set_r (gEqRefl hole) (semReturn hole)))

let class_method : atyp =
  let proof = gInject "arbitraryCorrect" in
  (set_full, hole, set_eq_set_incl_r proof)
...
let rec_method (inputs : arg list) (setinst : coq_expr) (moninst : coq_expr) (ih : var)
      (size : coq_expr) (n : int) (l : coq_expr list) : atyp =
  ...
  (iter_body l, gmon, proof)

let bind (opt : bool) (m : atyp) (x : string) (f : var -> btyp) : btyp =
  let (set, mon, sub) = m in
  let setf x =
    let (set, _, _) = f x in set
  in
  let monf x =
    let (_, mon, _) = f x in mon
  in
  let prf x =
    let (_, _, pr) = f x in pr
  in
  let hxc = "Hc_" ^ x in
  let hx = "H_" ^ x in
  let hcur' = "Hl_" ^ x in
  (set_bigcup x set setf,
   ...
   (fun hcur ->
      gMatch (imset_singl_incl hole hole hole hcur)
...
let ret (x : var) matcher =
  gImpl
    (gApp
       (imset (gInject "Some") matcher)
       [gVar x])
    hole

let ret_type_dec (x : var) (s : var) (left : coq_expr) (right : coq_expr) =
  ret x (gMatch (gVar s)
...
let check_expr (x : var) (n : int) (scrut : coq_expr) (left : btyp) (right : btyp) =
  let (lset, lmon, lproof) = left in
  let (rset, rmon, rproof) = right in
  ...
  (gMatchReturn scrut
   ...
   fun hcur ->
     gApp
       (gMatchReturn scrut
          "s" (* as clause *)
          (fun v -> ret_type_dec x v lset rset)
          [ (injectCtr "left", ["eq"] ,
...
let match_inp (x : var) (inp : var) (pat : matcher_pat) (left : btyp) (right : btyp) =
  ...
  in
  let proof_typ v =
    ret x (construct_match (gVar v) ~catch_all:(Some rset) [(pat, lset)])
  in
  (construct_match_with_return
     (gVar inp) ~catch_all:(Some rset) "s" (fun v -> hole)
     [(pat, lset)],
   construct_match_with_return
     (gVar inp) ~catch_all:(Some rmon) "s" mon_typ
     [(pat, lmon)],
   fun hcur ->
     gApp
       (construct_match_with_return
          (gVar inp) ~catch_all:(Some (rproof hcur)) "s" proof_typ
          [(pat, (lproof hcur))])
       [hcur]
  )
    ...
    | (chk, n) :: lst' ->
      ...
      gApp
        (gMatchReturn (chk (gVar x))
           "s" (* as clause *)
           (fun v -> ret_type_dec_prop v set_empty set)
           [ (injectCtr "left", ["heq"],
...
let genSizedSTCorr_body
  ...
  let handle_branch' (y : var) (ih : var) (size : coq_expr) (ins : var list) =
    handle_branch n dep_type ins
      ...
      (rec_method inputs setinst moninst ih size) bind (stMaybe y) (check_expr y) (match_inp y)
      gen_ctr (fun _ -> ())
  in
